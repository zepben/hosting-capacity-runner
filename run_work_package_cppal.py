    ...
    result = await eas_client.async_run_hosting_capacity_work_package(
        WorkPackageConfig(
            ...
            generator_config=GeneratorConfig(
                model=ModelConfig(
                    meter_placement_config=MeterPlacementConfig(
                        switch_meter_placement_configs=[SwitchMeterPlacementConfig(
                            ...
                            name_pattern="(?=.*LV.*)(?=.*[cC]ircuit.*)(?=.*[iI]solator.*)"
                        )]
                    ),
                    vmax_pu=1.35,
                    vmin_pu=0.8,
                    ...
                    max_single_phase_load=15_000.0,
                    max_load_service_line_ratio=2.0,
                    max_load_lv_line_ratio=2.0,
                    max_load_tx_ratio=3.5,
                    max_gen_tx_ratio=5.0,
                ...
                raw_results=RawResultsConfig(True, True, True, True, False)
            ),
